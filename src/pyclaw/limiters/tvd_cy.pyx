...
def limit_compiled(int num_eqn, np.ndarray[np.float64_t, ndim = 3] wave, np.ndarray[np.float64_t, ndim = 2] s,np.ndarray[np.int64_t, ndim = 1] limiter,np.ndarray[np.float64_t, ndim = 1] dtdx):
    r"""
    ...
    # as wave_norm2[i,j]
    cdef np.ndarray[np.float64_t, ndim=2] wave_norm2 = np.empty((wave.shape[1], wave.shape[2]), dtype=np.float64)
    for i in xrange(wave.shape[1]):
        for j in xrange(wave.shape[2]):
            wave_norm2[i, j] = (wave[0, i, j]**2) + (wave[1, i, j]**2) + (wave[2, i, j]**2)

    cdef np.ndarray[np.float64_t, ndim=2] wave_zero_mask = np.empty((wave.shape[1], wave.shape[2]), dtype=np.float64)
    cdef np.ndarray[np.float64_t, ndim=2] wave_nonzero_mask = np.empty((wave.shape[1], wave.shape[2]), dtype=np.float64)\

    for i in xrange(wave_norm2.shape[0]):
        for j in xrange(wave_norm2.shape[1]):
            wave_zero_mask[i, j] = (wave_norm2[i, j] == 0)
            wave_nonzero_mask[i, j] = 1.0 - wave_zero_mask[i, j]
    ...
    cdef np.ndarray[np.float64_t, ndim=3] tmp_dotls = np.empty((wave.shape[0], wave.shape[1], wave.shape[2] - 1), dtype=np.float64)
    ...
    cdef np.ndarray[np.float64_t, ndim=2] dotls = np.empty((tmp_dotls.shape[1], tmp_dotls.shape[2]), dtype=np.float64)
    ...
    cdef np.ndarray[np.float64_t, ndim=2] spos = np.empty((s.shape[0], s.shape[1] - 2), dtype=np.float64)
    ...
    cdef np.ndarray[np.float64_t, ndim=2] r = np.empty((s.shape[0], s.shape[1] - 2), dtype=np.float64)
    for i in xrange(s.shape[0]):
        for j in xrange(s.shape[1] - 2):
    ...
    for mw in xrange(wave.shape[1]):
        ...
        apply_wave_limiter(dtdx, mw, num_eqn, r, s, spos, wave, wave_nonzero_mask, wave_zero_mask)

    return wave

def limit(num_eqn,wave,s,limiter,dtdx):
    if use_cython: return limit_compiled(num_eqn,wave,s,limiter,dtdx)
...
@cython.boundscheck(False) # turn of bounds-checking for entire function
@cython.cdivision(True)
def apply_wave_limiter(np.ndarray[np.float64_t, ndim = 1] dtdx, int mw, int num_eqn, np.ndarray[np.float64_t, ndim = 2] r, np.ndarray[np.float64_t, ndim = 2] s, np.ndarray[np.float64_t, ndim = 2] spos, np.ndarray[np.float64_t, ndim = 3] wave, np.ndarray[np.float64_t, ndim = 2] wave_nonzero_mask, np.ndarray[np.float64_t, ndim = 2] wave_zero_mask):
    cdef np.ndarray[np.float64_t, ndim=1] cfl = np.empty(s[mw].shape[0] - 2, dtype=np.float64)
    cdef np.ndarray[np.float64_t, ndim = 1] wlimitr = np.empty(r[mw, :].shape, dtype=np.float64)
...
@cython.boundscheck(False) # turn of bounds-checking for entire function
def minmod_limiter_compiled(np.ndarray[np.float64_t, ndim = 1] r,np.ndarray[np.float64_t, ndim = 1] cfl):
    r"""
...
def minmod_limiter(r,cfl):
    if use_cython: return minmod_limiter_compiled(r, cfl)
    r"""
    Minmod vectorized limiter
    """
    a = np.ones((2,len(r)))
